package ariba::Ops::MarkLogic::ConfigParser;
...
use ariba::Ops::MarkLogic::HashMerge qw( merge );
use Data::Dumper;
...
##                for my $k (sort keys %$str) {
 ##           
  ##                  if (ref $data{$bn}->{$k} eq "ARRAY") {
   ##                     push @{$data{$bn}->{$k}}, $str->{$k};
    ##                }else{
     ##                   $data{$bn}->{$k} = $str->{$k};
      ##              }
       ##         }
       
                ariba::Ops::MarkLogic::HashMerge::set_behavior( 'RETAINMENT_PRECEDENT' );
                my %dataNew= %{ merge( $str, $data ) };
                $data = \%dataNew;

            }else{
                if ($subline =~ /\w+=\w+\s?/isg) {
                    my @lines = split('\n',$subline);
                    for(@lines){
                        if ($_ =~ /(\w+)=(.*)/isg) {
                            $data->{$key}->{$1} = [split(',',$2)];
                    ...
                    $subline =~ s/,\s*/,/g;
                    @result = split(',',$subline);
                    ##push @result , { table => $table} if ($table);
                    $data->{$key} = \@result;
                }
            }
        }   
...
sub as_string {
    ...
    my $STRING = undef;
    if ( $syntax eq 'ini' ) {
        $STRING .= "; ariba::Ops::MarkLogic::ConfigParser $VERSION\n";
        $STRING .= "; $currtime\n\n";
